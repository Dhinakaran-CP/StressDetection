...
class TestFeatureRuntimeLock(unittest.TestCase):
    def setUp(self):
        self.lock = FeatureRuntimeLock("contracts/feature_contract.yaml")
...
class TestDatasetCertifier(unittest.TestCase):
    def setUp(self):
        self.cert = DatasetCertifier("contracts/schema_contract.yaml")
    ...
    def test_certified_csvs_exist(self):
        for m in ("face", "voice", "physio"):
            p = f"dataset_certified/{m}_certified.csv"
            if os.path.exists(p):
...
class TestMultimodalPredict(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.loaded = cls.det.load_model("backend")
