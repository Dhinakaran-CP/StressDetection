...
def train_physio_expert():
    print("==============================================")
    print("STAGE 4: Training Physio Expert (LOSO Protocol)")
    print("==============================================")
    
    DATA_PATH = "dataset_certified/physio_certified.csv"
    ...
    feature_names = lock.contract["modalities"]["physio"]["features"]
    ...
    X_clean = []
    for row in X_raw:
        X_clean.append(lock.process_physio_features(row, scaler=None)[0])
    X = np.array(X_clean)
    ...
    # 5. Release
    package_and_release_expert("physio", model, scaler, X_test_scaled, y_test)
