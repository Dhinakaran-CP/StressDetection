...
class ConvMoE_MF(nn.Module):
    ...
    def __init__(self, hidden_dim=16, embed_dim=8, num_subjects=65,
                 num_datasets=3, face_dim=33, voice_dim=23, physio_dim=13,
                 grl_alpha_subj=0.02, grl_alpha_ds=0.02):
        super().__init__()
        self.embed_dim = embed_dim
        self.grl_alpha_subj = grl_alpha_subj
        self.grl_alpha_ds = grl_alpha_ds
        ...
        self.dataset_head    = nn.Linear(embed_dim, num_datasets)
    ...
    def forward(self, eye, mouth, global_face,
        ...
        rev_fused_subj = grad_reverse(fused, self.grl_alpha_subj)
        subj_logits = self.subj_head(rev_fused_subj)

        rev_fused_ds = grad_reverse(fused, self.grl_alpha_ds)
        dataset_logits = self.dataset_head(rev_fused_ds)
